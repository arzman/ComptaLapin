...
public class SynthAnnuelleDialog extends ComptaDialog {
    ...
    private void fillTable(int year) throws ComptaException {
        ...
        for (int month = 1; month < 13; month++) {
            ...
            rows.add(new String[]{ApplicationFormatter.moisFormat.format(date),
                    ApplicationFormatter.montantFormat.format(dep), ApplicationFormatter.montantFormat.format(res),
    ...
    private static class SynthTableModel extends AbstractTableModel {
        ...
        public Object getValueAt(int row, int col) {
            if (row < _data.size() && col < _data.get(row).length)
