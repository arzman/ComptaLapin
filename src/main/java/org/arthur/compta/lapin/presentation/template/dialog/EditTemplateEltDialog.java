...
public class EditTemplateEltDialog extends ComptaDialog {
    ...
    public EditTemplateEltDialog(TrimestreTemplateElement elt) {
        ...
        gbc.gridx = 0;
        gbc.gridy = row;
        content.add(new JLabel("Type :"), gbc);
        gbc.gridx = 1;
        content.add(_typeCombo, gbc);
        row++;
        gbc.gridx = 0;
        gbc.gridy = row;
        content.add(new JLabel("Fréquence :"), gbc);
        gbc.gridx = 1;
        content.add(_freqCombo, gbc);
        row++;
        gbc.gridx = 0;
        gbc.gridy = row;
        content.add(new JLabel("Occurence :"), gbc);
        gbc.gridx = 1;
        content.add(_occComb, gbc);
        row++;
        gbc.gridx = 0;
        gbc.gridy = row;
        content.add(new JLabel("Source :"), gbc);
        gbc.gridx = 1;
        content.add(_srcCombo, gbc);
        row++;
        gbc.gridx = 0;
        gbc.gridy = row;
        content.add(new JLabel("Cible :"), gbc);
        gbc.gridx = 1;
        content.add(_cibleCombo, gbc);
    ...
    private void initValues() {
        if (_templateElt != null) {
            ...
            if (_freqCombo.getItemCount() > 0)
                _freqCombo.setSelectedIndex(0);
            updateOccComb();
            if (_srcCombo.getItemCount() > 0)
                _srcCombo.setSelectedIndex(0);
            if (_cibleCombo.getItemCount() > 0)
                _cibleCombo.setSelectedIndex(0);
        }
    }
    ...
    private void checkInput(boolean changeOcc) {
        ...
        _montantTxt.setBorder(
                soldeOk ? UIManager.getBorder("TextField.border") : BorderFactory.createLineBorder(Color.RED));
