...
appop = new AppOperation(OperationDataAccess.getInstance().addOperation(libelle, montant, OperationType.DEPENSE,
EtatOperation.PREVISION, compteSrc.getAppId(), -1, appEm.getAppId()));
} else if (type.equals(OperationType.RESSOURCE.toString())) {
appop = new AppOperation(OperationDataAccess.getInstance().addOperation(libelle, montant, OperationType.RESSOURCE,
EtatOperation.PREVISION, compteSrc.getAppId(), -1, appEm.getAppId()));
} else if (type.equals(OperationType.TRANSFERT.toString())) {
appop = new AppTransfert(OperationDataAccess.getInstance().addOperation(libelle, montant, OperationType.TRANSFERT,
