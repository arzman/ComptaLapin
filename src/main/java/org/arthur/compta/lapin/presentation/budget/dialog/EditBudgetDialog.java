...
gbc.gridx = 0; gbc.gridy = row; content.add(new JLabel("Objectif : "), gbc);
gbc.gridx = 1; content.add(_objTxt, gbc); row++;

gbc.gridx = 0; gbc.gridy = row; content.add(new JLabel("Utilisé : "), gbc);
gbc.gridx = 1; content.add(_utilsTxt, gbc); row++;

gbc.gridx = 0; gbc.gridy = row; gbc.gridwidth = 2; content.add(_isRecurrentChk, gbc); row++;

gbc.gridwidth = 1;
gbc.gridx = 0; gbc.gridy = row; content.add(new JLabel("Date récurrence : "), gbc);
gbc.gridx = 1; content.add(_dateBudgetSpinner, gbc); row++;

gbc.gridx = 0; gbc.gridy = row; gbc.gridwidth = 2; content.add(_listBudRecuCB, gbc);
...
Double.parseDouble(_utilsTxt.getText().trim()), lblRec, dateRec);
} else {
...
boolean objOk = false;
try { Double.parseDouble(_objTxt.getText().trim()); objOk = true; } catch (NumberFormatException e) {}
...
try { Double.parseDouble(_utilsTxt.getText().trim()); utilOk = true; } catch (NumberFormatException e) {}
_utilsTxt.setBorder(utilOk ? UIManager.getBorder("TextField.border") : BorderFactory.createLineBorder(Color.RED));
