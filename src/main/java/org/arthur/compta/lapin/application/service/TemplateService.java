...
exMen.addOperation(new AppTransfert(OperationDataAccess.getInstance().addOperation(
elt.getNom(), elt.getMontant(), OperationType.valueOf(elt.getType()),
EtatOperation.PREVISION, elt.getCompteSource().getAppId(), elt.getCompteCible().getAppId(), exMen.getAppId())));
} else {
exMen.addOperation(new AppOperation(OperationDataAccess.getInstance().addOperation(
elt.getNom(), elt.getMontant(), OperationType.valueOf(elt.getType()),
EtatOperation.PREVISION, elt.getCompteSource().getAppId(), -1, exMen.getAppId())));
}
}
